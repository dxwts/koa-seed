 ...
 * @param {number} tm 尝试次数（因为有可能因网络故障超时），默认3次
 */
function url2base64(url, tm = 3) {
    let trytm = 1;
    return new Promise(resolve => {
        ...
        let clientReq = http.get(url, res => {
            ...
            res.on('end', err => {
                let data = Buffer.concat(chunks, chunks.size);
        ...
        clientReq.setTimeout(30000, async() => {
            if (trytm < tm) {
                let out = await url2base64(url, trytm + 1);
                resolve(out);
            } else {
                resolve('timeout');
            }
